...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (id) {
        ...
        .then(data => {
            if (data.success) {
                const app = data.data;
                document.title = "Application - " + app.ref_number;
                ...
                if (app.prev_marks_doc) {
                    let docSrc = app.prev_marks_doc;
                    if (docSrc.indexOf('assets/') !== -1) {
                        docSrc = '/' + docSrc.substring(docSrc.indexOf('assets/'));
                    }
                    document.getElementById('view_doc_btn').href = docSrc;
                    document.getElementById('view_doc_btn').style.display = 'inline-block';
                }
