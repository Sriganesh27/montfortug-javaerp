...
public class StudentController {
    ...
    private Long getCurrentUserBranchId() {
        ...
        try {
            ...
            return jdbcTemplate.queryForObject("SELECT branch_id FROM erp_users WHERE user_id = ?", Long.class, userId);
        } catch (Exception e) {
            return null;
        }
    }
    ...
    public ResponseEntity<?> admitStudent(HttpServletRequest request, @RequestParam Map<String, String> formData) {
        ...
        try {
            ...
            if (name == null || name.isEmpty() || surname == null || surname.isEmpty() || rawClass == null || rawClass.isEmpty()) {
                return ResponseEntity.ok(Map.of("success", false, "message", "Name, Surname, and Class are required."));
            }
            ...
            return ResponseEntity.ok(Map.of("success", true, "message", "Student Admitted Successfully!<br><strong>Adm-no: " + String.format("%04d", adNo) + "</strong><br><strong> Student ID: " + username + "</strong>"));
