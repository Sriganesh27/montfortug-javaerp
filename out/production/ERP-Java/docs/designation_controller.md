...
Here is the exact code for the `DesignationController`. As requested, I have **dropped the `v1`** from the path, so it simply maps to `/api/designations`.

---

### `DesignationController.java`
**Location:** `src/main/java/com/erp/montfortuganda/school/controller/DesignationController.java`
...
> [!TIP]
> Make sure to open your existing `DepartmentController.java` and change `@RequestMapping("/api/v1/departments")` to `@RequestMapping("/api/departments")` to keep them consistent!
